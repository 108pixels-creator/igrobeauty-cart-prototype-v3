...
(() => {
  ...
  function collectProps(node, kind, host) {
    ...
    for (const { name, value } of [...node.attributes]) {
      ...
      if (key.startsWith("data-dc-") && key !== "data-dc-s") {
        pseudoClasses.push(host.pseudoClass(key.slice(8), value));
        continue;
      }
      if (key === "data-dc-s") key = "style";
  ...
  function walkComponent(el, host) {
    ...
    const styleRaw = el.getAttribute("style") ?? el.getAttribute("data-dc-s");
